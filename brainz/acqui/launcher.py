...
class AcquiLauncher(threading.Thread):
    ...
    def init_scheduler(self):
        ...
        painters = [clazz.ClazzPainter(self.screen, cl, 140, [])
                    for cl in self.cnf['classes']]
        ...
        # idle
        taskIdle = tasks.Task(duration_provider_factory(
            self.cnf['durations']['idle']))
        taskIdle.addAction(bg)
        self.schedulerRunning.addTask(taskIdle)
        # show fix
        taskFix = tasks.Task(duration_provider_factory(
            self.cnf['durations']['fixation']))
        ...
        # show class
        taskClass = tasks.Task(duration_provider_factory(
            self.cnf['durations']['class'] + 300))
...
def duration_provider_factory(duration):
    ...
    logging.getLogger("logger").debug("duration %s" %
